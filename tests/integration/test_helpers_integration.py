...
def _has_typst_and_muchpdf() -> bool:
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            res2 = subprocess.run([
                'typst', 'compile', '--root', str(PROJECT_ROOT.resolve()),
...
class TestHelpersIntegration(unittest.TestCase):
    ...
    def test_cli_pdf_compile_with_helpers_if_available(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            cmd = [
                sys.executable, '-m', 'pagemaker.cli', 'pdf', str(org_path),
                '--export-dir', td, '--pdf-output', 'out.pdf', '--no-clean'
            ]
            env = os.environ.copy()
            env['PYTHONPATH'] = str(SRC_PATH) + os.pathsep + env.get('PYTHONPATH', '')
            res = subprocess.run(cmd, cwd=str(PROJECT_ROOT), env=env, capture_output=True, text=True)
            if res.returncode != 0:
                self.fail(f"CLI PDF compile with helpers failed. STDOUT:\n{res.stdout}\nSTDERR:\n{res.stderr}")
