...
class TestCustomStyles(unittest.TestCase):
    def test_meta_style_overrides_core_types(self):
        ir = {
            ...
            'pages': [{
                ...
                'elements': [{
                    'id': 'h', 'type': 'header',
                    'area': {'x': 1, 'y': 1, 'w': 6, 'h': 1}, 'z': 10,
                    'text_blocks': [{'kind': 'plain', 'content': 'My Header'}],
                    'style': None,
                }, {
                    'id': 'b', 'type': 'body',
                    'area': {'x': 1, 'y': 2, 'w': 6, 'h': 1}, 'z': 10,
        ...
        self.assertIn('#text(font: "Shree714", weight: 900, size: 30pt, fill: rgb("#ff00aa"))[My Header]', typst)
        self.assertIn('#text(font: "Manrope", fill: rgb(50%,50%,50%))[Body text]', typst)
    ...
    def test_custom_named_style_and_element_style_reference(self):
        ...
        typst = pm.generate_typst(ir)
        self.assertIn('#text(font: "Manrope", weight: "bold", size: 36pt, fill: rgb("#123456"))[Big Title]', typst)
